(function (root) {
  ...
  function create(options = {}) {
    ...
    function syncUi() {
      ...
      if (structural && elements.properties) {
        ...
        if (selection.kind === "image" && panel) {
          ...
          const cropEditable = imagePanels.length === 1 && panelEditable(panel) && !panel.image_locked && Boolean(panel.image_id);
    ...
    async function importProjectImages(records) { for (const record of Array.isArray(records) ? records : []) { if (!String(record?.id || "").startsWith(IMAGE_PREFIX) || !String(record.data_url || "").startsWith("data:image/")) continue; const blob = await fetch(record.data_url).then((response) => response.blob()); let metadata = comic.images.find((item) => item.id === record.id); if (!metadata) { metadata = { id: String(record.id), name: String(record.name || "project-image"), mime: record.mime || blob.type || "image/png", width: 1, height: 1, sha256: "", source: "stored" }; comic.images.push(metadata); } await attachBlob(metadata, blob, false); await imageStore.put(metadata, blob); } renderTray(); requestRender({ canvas: true, layers: true }); }
    ...
    async function importExternalImage(file, serverAsset, control = {}) {
      ...
      if (!runtimeImages.has(existing.id)) await attachBlob(existing, file, false);
