(function (root) {
  ...
  function create(options = {}) {
    ...
    async function importFiles(files, importOptions = {}) {
      ...
      for (const file of supported) {
        ...
        try {
          ...
          if (storedDuplicate) {
            if (!runtimeImages.has(storedDuplicate.id)) await attachBlob(storedDuplicate, file, false);
    ...
    function handleContextMenu(event, point) {
      ...
      menu.querySelector('[data-comic-context="edit-crop"]').disabled = !panel.node.image_id || panel.node.image_locked;
      menu.querySelector('[data-comic-context="reset-crop"]').disabled = !panel.node.image_id || panel.node.image_locked || panelImageCropIsFull(panel.node);
      menu.hidden = false;
      options.requestRender({ canvas: true });
      return true;
    }
